...
public class JournalEntryControllerV3 {
    ...
    public boolean createEntry(@RequestBody JournalEntryV2 myEntry) {
...
//        return journalEntryService.findById(myId).orElse(null); if Optional is returned
        ...
        Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
        String userName = authentication.getName();
        /// the user should get his entry only, if he can login to system and he got id of any other user's journal
        /// entry then he can access his entry using that id. So we need to check this

        User user = userService.findByUserName(userName); // get the current user
        // if the myId is not in list of JournalEntries of this user, it means we r sending wrong id of journal

        List<JournalEntryV2> collect = user.getJournalEntries().stream()
